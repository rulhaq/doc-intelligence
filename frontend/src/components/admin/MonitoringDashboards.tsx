...
export default function MonitoringDashboards() {
  ...
  const handleDashboardChange = (dashboard: typeof dashboards[0]) => {
  ...
  // Utility function for formatting bytes (used for future storage display)
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            ...
            <div className={`p-2 rounded-lg ${
              stats.ollama_status === 'healthy' ? 'bg-blue-100' : 'bg-red-100'
            }`}>
              <CpuChipIcon className={`w-6 h-6 ${
                stats.ollama_status === 'healthy' ? 'text-blue-600' : 'text-red-600'
              }`} />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-blue-700 mt-1">
              If dashboards don't load in the iframe due to security restrictions, click "Open in New Tab" to access them directly.
              Make sure all services are running: <code className="bg-blue-100 px-1 rounded">docker compose ps</code>
            </p>
          </div>
        </div>
      </div>
